...
class ManagerController(
    ...
    ) = searchManagersUseCase.searchManagers(SearchManagersCommand(
        searchKeys = search.searchFields.map { ManagerSearchField.valueOf(it.name) }.toSet(),
        searchText = search.searchText,
        pageQuery = PageQuery(
            page = page.pageNumber,
            pageSize = page.pageSize,
            sort = page.sort.map { SortOption(it.property, SortDirection.valueOf(it.direction.toString())) }.toList(),
        )
    ))
