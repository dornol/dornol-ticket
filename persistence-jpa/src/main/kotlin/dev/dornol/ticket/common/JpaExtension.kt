...
fun Sort.toOrderBy(orderSpecifierSupplier: (sort: Sort.Order) -> OrderSpecifier<out Comparable<*>>?) = this.map { orderSpecifierSupplier(it) }.filterNotNull().toTypedArray()

infix fun <T: BooleanExpression?> T.and(that: T) = this?.and(that) ?: that
infix fun <T: BooleanExpression?> T.or(that: T) = this?.or(that) ?: that

fun <T : Enum<T>> SearchBaseCriteria<T>.textSearch(mapper: Map<T, List<StringPath>>): BooleanExpression? {
